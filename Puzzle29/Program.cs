...
namespace Puzzle29
{
    class Program
    {
        static async Task Main()
        {
            ...
            var risk = FindLeastRiskyPath(map);
            
            Console.WriteLine($"Path risk: {risk}");
        }

        private static IReadOnlyDictionary<(int y, int x), int> ParseInput(string input)
        {
            return input
                .Split('\n')
        ...
        private static int FindLeastRiskyPath(IReadOnlyDictionary<(int y, int x), int> map)
        {
        ...
        private static void UpdateTotalRisk((int y, int x) cell, Func<(int y, int x), bool> isWithinBoundary, IReadOnlyDictionary<(int y, int x), int> map, (int y, int x) current, Dictionary<(int y, int x), int> totalRiskMap, HashSet<(int y, int x)> partiallyVisitedCells)
        {
